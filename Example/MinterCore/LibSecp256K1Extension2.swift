//
//  LibSecp256K1Extension2.swift
//  MinterCore_Example
//
//  Created by Alexey Sidorov on 14/03/2018.
...
extension SECP256K1_2 {
    ...
    static func signForRecovery(hash: Data, privateKey: Data) -> (compressed: Data?, uncompressed: Data?) {
        if hash.count != 32 || privateKey.count != 64 { return (nil, nil) }
        if !SECP256K1_2.verifyPrivateKey(privateKey: privateKey) {
            return (nil, nil)
        }
        for rounds in 0 ... 1024 {
            ...
            var uncompressed: secp256k1_ecdsa_recoverable_signature = .init()
            ...
            guard let recoveredPubKey = SECP256K1_2.recoverPublicKey(hash: hash, signature: compressedSignature) else { continue }
            guard let truePubKey = SECP256K1_2.privateToPublic(privateKey: privateKey) else { continue }
    ...
    static func privateToPublic(privateKey: Data, compressed: Bool = false) -> Data? {
        if privateKey.count != 64 { return nil }
    ...
    static func recoverPublicKey(hash: Data, signature: Data, compressed: Bool = false) -> Data? {
        ...
        var uncompressed = secp256k1_ecdsa_recoverable_signature()
    ...
    static func recoverSender(hash: Data, signature: Data) -> EthereumAddress? {
        guard let pubKey = SECP256K1_2.recoverPublicKey(hash: hash, signature: signature, compressed: false) else { return nil }
    ...
    static func verifyPrivateKey(privateKey: Data) -> Bool {
        if privateKey.count != 64 { return false }
