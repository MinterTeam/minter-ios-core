...
public class DeclareCandidacyRawTransaction : RawTransaction {
	...
	public convenience init(nonce: BigUInt, gasCoin: Data, data: Data) {
		...
		self.init(nonce: nonce, gasPrice: BigUInt(1), gasCoin: gasCoin, type: RawTransactionType.declareCandidacy.BigUIntValue(), payload: Data(), serviceData: Data())
		self.data = data
	}
	
	public convenience init(nonce: BigUInt, gasCoin: String, address: String, publicKey: String, commission: BigUInt, coin: String, stake: BigUInt) {
...
public struct DeclareCandidacyRawTransactionData : Encodable, Decodable {
	...
	public init(address: String, publicKey: String, commission: BigUInt, coin: String, stake: BigUInt) {
