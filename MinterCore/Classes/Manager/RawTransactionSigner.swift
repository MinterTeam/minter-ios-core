...
public enum RawTransactionSigner {
    /**
    ...
    public static func publicKey(privateKey: Data, compressed: Bool = false) -> Data? {
        ...
        if !compressed {
            publicKey = [UInt8](repeating: 0x00, count: 65)
            outputLength = Int(65)
        } else {
            publicKey = [UInt8](repeating: 0x00, count: 33)
            outputLength = Int(33)
        }
    ...
    public static func proof(address: String, passphrase: String) -> Data? {
        ...
        guard sig.r != nil, sig.s != nil, sig.v != nil else {
            return nil
        }
