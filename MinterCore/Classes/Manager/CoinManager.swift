...
public class CoinManager: BaseManager {
     ...
     */
    public func info(symbol: String, height: String = "0", completion: ((Coin?, Error?) -> Void)?) {
        let url = MinterAPIURL.coinInfo.url()

        httpClient.getRequest(url, parameters: ["symbol": symbol, "height": height]) { response, error in
