...
public class TransactionManager: BaseManager {
     ...
     */
    public func transaction(hash: String, completion: ((Transaction?, Error?) -> Void)?) {
        let url = MinterAPIURL.transaction.url()

        httpClient.getRequest(url, parameters: ["hash": hash]) { response, error in
    ...
    public func transaction(query: String, completion: (([Transaction]?, Error?) -> Void)?) {
        let url = MinterAPIURL.transactions.url()

        httpClient.getRequest(url, parameters: ["query": query]) { response, error in
    ...
    public func unconfirmedTransaction(limit: String = "0", completion: (([String: Any]?, Error?) -> Void)?) {
        let url = MinterAPIURL.unconfirmedTransactions.url()

        httpClient.getRequest(url, parameters: ["limit": limit]) { response, error in
     ...
     */
    public func send(tx: String, completion: ((String?, String?, Error?) -> Void)?) {
        let url = MinterAPIURL.sendTransaction.url()

        httpClient.getRequest(url, parameters: ["tx": tx]) { response, error in
     ...
     */
    public func estimateCoinBuy(from: String, to: String, amount: Decimal, completion: ((Decimal?, Decimal?, Error?) -> Void)?) {
        ...
        httpClient.getRequest(url, parameters: ["coin_to_sell": from, "coin_to_buy": to, "value_to_buy": value]) { response, error in
            ...
            if let estimatePayload = response.data as? [String: Any], let willGet = estimatePayload["will_pay"] as? String, let commission = estimatePayload["commission"] as? String {
                ...
                guard willPay != nil && com != nil else {
     ...
     */
    public func estimateCoinSell(from: String, to: String, amount: Decimal, completion: ((Decimal?, Decimal?, Error?) -> Void)?) {
        ...
        httpClient.getRequest(url, parameters: ["coin_to_sell": from, "coin_to_buy": to, "value_to_sell": value]) { response, error in
            ...
            if let estimatePayload = response.data as? [String: Any], let willGet = estimatePayload["will_get"] as? String, let commission = estimatePayload["commission"] as? String {
                ...
                guard willGet != nil, com != nil else {
                    err = CoinManagerError.noEstimate
                    return
                }
    ...
    public func estimateCommission(for rawTx: String, height: String = "0", completion: ((Decimal?, Error?) -> Void)?) {
        let url = MinterAPIURL.estimateTxCommission.url()

        let tx = rawTx.stripMinterHexPrefix()

        httpClient.getRequest(url, parameters: ["tx": "Mt" + tx, "height": height]) { response, _ in
    ...
    /**
     ...
     - Precondition: `from` and `to` should be uppercased (e.g. MNT, BLTCOIN, etc.)
     */
    public func estimateCoinSellAll(from: String, to: String, gasPrice: Int = 1, amount: Decimal, completion: ((Decimal?, Decimal?, Error?) -> Void)?) {
        let url = MinterAPIURL.estimateCoinSell.url()
        ...
        httpClient.getRequest(url, parameters: ["coin_to_sell": from, "coin_to_buy": to, "value_to_sell": value, "gas_price": gasPrice]) { response, error in
