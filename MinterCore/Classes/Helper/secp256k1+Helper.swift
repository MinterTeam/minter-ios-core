...
public class SECP256k1Helper: NSObject {
    var secp256k1Context: OpaquePointer
    var privateKey = UnsafePointer<UInt8>(bitPattern: 0)
    ...
    func privateKeyTweakAdd(tweak: Data) throws {
        ...
        _ = tweak.withUnsafeBytes { uint8Pointer in
            result = Int(secp256k1_ec_privkey_tweak_add(secp256k1Context, UnsafeMutablePointer<UInt8>(mutating: self.privateKey!), uint8Pointer)) as NSNumber
        }
        if !result.boolValue {
            throw keyError.tweakFailed
        }
    ...
    func privateKeyTweakMul(tweak: Data) throws {
        ...
        var result = NSNumber(value: 0)
        _ = tweak.withUnsafeBytes { uint8Pointer in
            result = Int(secp256k1_ec_privkey_tweak_mul(secp256k1Context, UnsafeMutablePointer<UInt8>(mutating: self.privateKey!), uint8Pointer)) as NSNumber
        }
        if !result.boolValue {
            throw keyError.tweakFailed
        }
    ...
    func publicKeyNegate() throws {
        var result = NSNumber(value: 0)
        result = secp256k1_ec_pubkey_negate(secp256k1Context, publicKey) as NSNumber
        if !result.boolValue {
    ...
    func privateKeyNegate() throws {
        ...
        var result = NSNumber(value: 0)
        result = secp256k1_ec_privkey_negate(secp256k1Context, UnsafeMutablePointer<UInt8>.init(mutating: privateKey!)) as NSNumber
        if !result.boolValue {
            throw keyError.keyNegationFailed
        }
    }
